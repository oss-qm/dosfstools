...
static int invariant = 0;		/* Whether to set normally randomized or
					   ...
					   constants */
static int fill_mbr_partition = 0;	/* Whether to fill MBR partition table or not */
...
static void setup_tables(void)
{
    ...
    if (fill_mbr_partition) {
        ...
        /* 32 bit disk signature - random number */
        if (invariant)
            disk_sig = volume_id;
        else
...
static void usage(const char *name, int exitval)
{
    ...
    fprintf(stderr, "  --mbr           Fill (fake) MBR table with one partition which spans whole disk\n");
